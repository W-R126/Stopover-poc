...
export default class Month extends React.Component<MonthProps, MonthState> {
  ...
  private getPrice(month: Date | undefined): JSX.Element | null {
    if (!month) { return null; }

    const { datePrice } = this.props;
    if (!datePrice) return null;

    const findPrice = datePrice.find((item) => item.date.getDate() === month.getDate());
    if (findPrice) {
      const classList = [css.Price];
      if (findPrice.date.getDay() === 0) { classList.push(css.Sunday); }
      return <span className={classList.join(' ')}>{findPrice.price}</span>;
    }
    return null;
  }
